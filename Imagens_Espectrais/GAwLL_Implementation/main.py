...
def print_importance_sorted(importance_values, min_values, feature_names):
    print("\n=== IMPORTÂNCIA DAS CARACTERÍSTICAS (MÁXIMA E MÍNIMA) ===\n")

    # Criamos trios com (nome, valor_maximo, valor_minimo)
    # importance_values aqui é tratado como o valor máximo/principal
    trios = list(zip(feature_names, importance_values, min_values))
    
    # Ordenamos pelo valor absoluto da importância principal (máxima)
    trios.sort(key=lambda x: abs(x[1]), reverse=True)

    # Cabeçalho para facilitar a leitura
    print(f"{'Característica':<40} | {'Máxima':>12} | {'Mínima':>12}")
    print("-" * 70)

    for name, max_val, min_val in trios:
        print(f"{name:<40} | {max_val:>12.6f} | {min_val:>12.6f}")

def print_top_interactions(interaction_matrix, min_interaction_matrix, feature_names, top_n=10):
    print(f"\n=== TOP {top_n} INTERAÇÕES (MÁXIMAS E RESPECTIVAS MÍNIMAS) ===\n")
    ...
    for i in range(n):
        for j in range(i + 1, n):
            # Valor da interação principal (máxima)
            max_val = interaction_matrix[i][j]
            # Valor da interação mínima correspondente
            min_val = min_interaction_matrix[i][j]
            
            interactions.append((feature_names[i], feature_names[j], max_val, min_val))

    # Ordena pelo valor absoluto da interação máxima
    interactions.sort(key=lambda x: abs(x[2]), reverse=True)

    # Formatação das colunas para melhor leitura
    print(f"{'Característica 1':<30} | {'Característica 2':<30} | {'Máxima':>10} | {'Mínima':>10}")
    print("-" * 95)

    for f1, f2, max_v, min_v in interactions[:top_n]:
        print(f"{f1:<30} | {f2:<30} | {max_v:>10.6f} | {min_v:>10.6f}")
